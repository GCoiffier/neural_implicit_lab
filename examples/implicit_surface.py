...
if geometry.dim == 3:
    points, normals = M.sampling.sample_surface(geometry, 100_000, return_normals=True)
elif geometry.dim == 2:
...
if geometry.dim == 2:
    ...
    trainer.add_callbacks(callbacks.MarchingCubeCB("output", 50, res=300, iso=0.))
trainer.set_training_data(train_data)
trainer.train(model)

# IL.nn.save_model(model, "output/model.pt")
torch.save(model.state_dict(), "siren_64x6.pt")